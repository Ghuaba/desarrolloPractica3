...
class LoteControl:
    ...
    def buscarExternal(self, external):
        return Lote.query.filter_by(external_id = external).all()
    ...
    def listarEstadosLote(self):
        estados = {estado.name: estado.value for estado in EstadoLote}
        return estados
...
#----------------------------------------------------------------
    ...
    def saveProductoLote(self, data, imagen):
        ...
        lote.fecha_caducidad = fecha_caducidad
        lote.cantidad = data['cantidad']
        lote.direccion_imagen =  nombre_imagen.filename
        lote.precio = data['precio']
        ...
        db.session.add(lote)
        db.session.commit()
        nombre_imagen.save(os.path.join(current_app.config['PATHCARPETA'], lote.direccion_imagen))
    ...
    def modify(self, external_id, data):
        ...
        new_lote.fecha_emision = data.get('fecha_emision', lote.fecha_emision)
        new_lote.codigo = data.get('codigo', lote.codigo)
        new_lote.fecha_caducidad = data.get('fecha_caducidad', lote.fecha_caducidad)
        new_lote.precio = data.get('precio', lote.precio)
        new_lote.cantidad = data.get('cantidad', lote.cantidad)
    ...
    def obtenerImagenLote(self, external_id):
        lote = Lote.query.filter_by(external_id=external_id).first()
        if not lote:
            return None  # Devolver None si no se encuentra el lote
        return lote.direccion_imagen
